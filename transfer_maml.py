...
def train(model, train_data, evaluate_fips, train_fips, epoch, iter_count=0, mode="init", exp_postfix="", best_train_r2 = float("-inf")):
    ...
    progress = ProgressMeter(len(train_data_batch), [batch_time, data_time, losses, r2],
                                 prefix="Train [{}] Model Epoch: [{}]".format(mode, epoch))
    ...
    for inner_epoch in range(args.num_inner_epochs):
        ...
        for iter, batch in enumerate(train_data_batch):
            ...
            if iter % 1 == 0:
                ...
                # if mode == "init":
                #     step = iter + len(train_data_batch) * epoch
            ...
            # easy/hard model use parts of data, so need to evaluate whole train data
        R2, df, pred_res, gold_res = evaluate(train_data, evaluate_fips, maml, mode, epoch, iter_count, exp_postfix=exp_postfix, use_iter=True)
    ...
    ckpt_name = 'model_epoch_' + str(epoch) + f'_iter_{iter_count}.pkl'
...
def test(test_data, test_fips, epoch, mode="init", exp_postfix=""):
    lossfn = nn.MSELoss(reduction='mean')
    compute_r2 = R2Loss()
    helper = HelperFunctions()
    convert_index_corn = 0.429
...
def evaluate(data, fips, maml, mode, epoch, iter_count, exp_postfix="", use_iter=False):
    ...
    r2 = AverageMeter('R2', ':.4e')
    progress = ProgressMeter(len(test_data_batch), [batch_time, data_time, losses, r2],
                             prefix="Evaluate [{}] Model Epoch: [{}] Iter: [{}]".format(mode, epoch, iter_count))
...
def pretrain(args, train_data, train_fips):
    ...
    for epoch in range(args.pretrain_epochs):
        ...
        for iter, batch in enumerate(train_data_batch):
            ...
            support_x, support_y, support_fips, support_dp, support_year, query_x, query_y, query_fips, query_dp, query_year = batch[0], batch[1], batch[2], batch[3], batch[4], batch[5], batch[6], batch[7], batch[8], batch[9]
...
def main():
    ...
    model = pretrain(args, train_data, train_fips)
    ...
    for epoch in range(args.num_epochs):
        ...
        for phase in phases:
            if phase == "train":
                _, train_task_loss = train(model, train_data, train_fips, train_fips, epoch, exp_postfix=exp_postfix)

            if phase == "test":
                test(test_data, test_fips, epoch, exp_postfix=exp_postfix)
...
def get_argparser():
    ...
    parser.add_argument('--pretrain-epochs', default=5, type=int, help='number of pretrain running epochs')
